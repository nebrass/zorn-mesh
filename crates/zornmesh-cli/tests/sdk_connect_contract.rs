...
use zornmesh_cli::daemon::{DaemonConfig, DaemonRuntime};
use zornmesh_cli::sdk::{ConnectOptions, Mesh, SdkErrorCode};
...
fn unique_socket(name: &str) -> PathBuf {
    ...
    let short_name: String = name
        ...
        .collect();
    PathBuf::from("/tmp")
        .join(format!("zms{short_name}-{}-{nanos}", std::process::id()))
        .join("z")
}
